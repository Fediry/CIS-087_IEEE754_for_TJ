...
uint32_t conv_mantissa(uint32_t raw_mantissa) {}
...
float ieee754::ieee_754(uint32_t const data) {
    ...
    cout << " mantissa: " << raw_mantissa;

    uint32_t mantissa = conv_mantissa(raw_mantissa);
    ...
    value = (-1 ^ sign_bit) * (2 ^ exponent) * (leading_m_bit + mantissa);
